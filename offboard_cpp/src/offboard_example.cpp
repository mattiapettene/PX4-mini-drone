...
public:
	...
	// Create subscribers
	this->vehicle_status_sub_ = this->create_subscription<VehicleStatus>("/fmu/out/vehicle_status",qos_sub, std::bind(&OffboardControl::vehicle_status_clbk, this, _1));

	// get common timestamp
	this->timesync_sub_ = this->create_subscription<TimesyncStatus>("fmu/timesync/out", qos_sub, [this](const TimesyncStatus::UniquePtr msg)
	{
		timestamp_.store(msg->timestamp);
	});

	// Create publishers
	this->offboard_control_mode_publisher_ = this->create_publisher<OffboardControlMode>("/fmu/in/offboard_control_mode", qos_pub);
	this->trajectory_setpoint_publisher_ = this->create_publisher<TrajectorySetpoint>("/fmu/in/trajectory_setpoint", qos_pub);
	this->vehicle_command_publisher_ = this->create_publisher<VehicleCommand>("/fmu/in/vehicle_command", qos_pub);
...
private:
	...
	void vehicle_status_clbk(const VehicleStatus & msg)
	{
